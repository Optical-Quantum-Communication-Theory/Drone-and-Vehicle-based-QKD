function [newParams,modParser] = ThreeStateDataTableDifInt_Desc(params, options, debugInfo)
...
pAandGen = [0, 119/256, 119/256]; % p(a,gen)  
pAandTest = [3/128, 3/128, 3/128]; % p(a,gen)
pGen = sum(pAandGen); 
pTest = sum(pAandTest); 
...
newParams.pMuandACondTest = pMuandACondTest;%probSignalsAtest.*cell2mat(params.decoyProbs); %p(mu,a|test)
