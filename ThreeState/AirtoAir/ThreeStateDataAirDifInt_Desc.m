function [newParams,modParser] = ThreeStateDataAirDifInt_Desc(params, options, debugInfo)
...
%Flight 1&2
ketR = [0.649072591296342; -0.335267880711076 + 0.682861786448294*1i];
ketL = [0.724292218484579; 0.311604649784240 - 0.615063675139029*1i];
ketH = [0.999991175428320; -0.002599931168138 - 0.003299912636483*1i];
...
%Flight 1& 2
pAandGen = [0, 1/4*119/121, 1/4*119/121];
pAandTest = [1/3, 127/1452, 127/1452]; 
...
debugInfo.storeInfo("pMuandACondTest", pMuandACondTest);%probSignalsAtest.*cell2mat(params.decoyProbs)); 
...
krausOpXAlice = sqrt(pGenCondSingleandA(2))*zket(2,1)*zket(3,2)' + sqrt(pGenCondSingleandA(3))*zket(2,2)*zket(3,3)'; 
%krausOpX = kron(krausOpX, sqrtm(GammaR + GammaL)); %only comp basis used for key gen
krausOpX = kron(krausOpXAlice, sqrtRL);
krausOps = {krausOpX}; 
