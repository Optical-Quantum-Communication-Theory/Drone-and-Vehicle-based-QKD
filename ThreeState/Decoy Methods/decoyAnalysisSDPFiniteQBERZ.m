...
function [Y1L,Y1U] = decoyAnalysisSDPFiniteQBERZ(expecJointTest,statesAlice,POVMsBob,decoys,probListAlice,probsDecoy,ptest,photonBound,t,mu,options)
    arguments
    ...
    probHCondDecoy = sum(expecJointTest(1,:,:),[1,2]);

    %QBER conditioned on each intensity
    qberAndDecoy = squeeze(expecJointTest(1,2,:));
    tqber = squeeze(t(1,2,:));
    muqber = squeeze(mu(1,2,:));

    %Gain conditioned on each intensity
    gainAndDecoy = qberAndDecoy + squeeze(expecJointTest(1,1,:) );
    tgain = tqber + squeeze(t(1,1,:));
    mugain = muqber + squeeze(mu(1,1,:));

    %Calculate upper and lower bounds on the expectations    
    upperbndsQber = min(max((qberAndDecoy + tqber + muqber)./((probsDecoy.*sum(probListAlice(1)).*ptest)),0),1);
    lowerbndsQber = min(max((qberAndDecoy - tqber - muqber)./(probsDecoy.*sum(probListAlice(1)).*ptest),0),1);
    
    upperbndsGain = min(max((gainAndDecoy + tgain + mugain)./(probsDecoy.*sum(probListAlice(1)).*ptest),0),1);
    lowerbndsGain = min(max((gainAndDecoy - tgain - mugain)./(probsDecoy.*sum(probListAlice(1)).*ptest),0),1);
                ...
                cvx_begin quiet
                    ...
                    subject to
                        ...
                        if indexopt == 1 %QBER
                            ...
                            Y0 <= 1/2;
                            qberchannel1 = trace(J1*kron(POVMsBob{2},transpose(rhoAlice{1})));
                            norm(Y1 - qberchannel1) <= choi_tolerance;
    
                            qberchannel0 = trace(J0*kron(POVMsBob{2},1));
                            ...
                            gainchannel1 = trace(J1*kron(POVMsBob{1},transpose(rhoAlice{1}))) + trace(J1*kron(POVMsBob{2},transpose(rhoAlice{1}))) ;
                            norm(Y1 - gainchannel1) <= choi_tolerance;
    
                            gainchannel0 = trace(J0*kron(POVMsBob{1},1)) + trace(J0*kron(POVMsBob{2},1));
